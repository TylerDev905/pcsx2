...
void Convert_Line(unsigned int code, char* result)
{
	...
	if (hack)
		String_Replace(result, (char*)"rt", (char*) registers[GET_RS(code)]);
...
void Format_String(char* string, unsigned int code)
{
	...
	String_Replace(string, (char*)"rs", (char*)   registers[GET_RS(code)]);
	String_Replace(string, (char*)"rd", (char*)   registers[GET_RD(code)]);
	String_Replace(string, (char*)"rt", (char*)   registers[GET_RT(code)]);
	String_Replace(string, (char*)"fs", (char*) f_registers[GET_RS(code)]);
	String_Replace(string, (char*)"fd", (char*) f_registers[GET_RD(code)]);
	String_Replace(string, (char*)"ft", (char*) f_registers[GET_RT(code)]);

	Hex_to_String(GET_IMMEDIATE(code), immediate_value, 4);
	String_Replace(string, (char*)"_i", immediate_value);
}
