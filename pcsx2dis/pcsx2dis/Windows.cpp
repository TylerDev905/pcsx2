...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hNull, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	AppendMenu(menu_analyse, MF_ENABLED | MF_STRING, 30, "Analyse Al");
		...
		case WM_SIZING:
		{
			if (hwnd == main_hwnd && listbox_hwnd != NULL && wndBreak.hwnd != NULL)
			{
				...
				for (int i = 0; i < list.maxitems; i ++)
					SendMessage(listbox_hwnd, LB_ADDSTRING, 0, (LPARAM) "NUL");
			}
		}
		break;
		case WM_KEYDOWN:
...
void CreateGameSharkWindow()
{
	...
	//gs->btCancel = CREATE("BUTTON", "Cance", WS_CHILD | BS_PUSHBUTTON, 244, 293, 75, 23, gs->gbActiveCodes);
...
void Do_CreateStruct(int editingId)
{
	...
	wndStruct.btCancel = CreateWindowEx(0, "BUTTON", "Cance", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 94, 290, 75, 23, wndStruct.hwnd, NULL, globalInst, 0);
